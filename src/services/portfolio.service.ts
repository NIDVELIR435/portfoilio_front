...
export class PortfolioService {
    ...
    public async create(requestBody: Omit<Portfolio, "id" | "createdAt" | "updatedAt">): Promise<ResponseType> {
        try {
            const response = await this.agent.post<Portfolio>('portfolio/create', JSON.stringify(requestBody))
    ...
    public findAll(): Promise<Portfolio[]> {
        return this.agent.get<Portfolio[]>('portfolio/all').then(({data})=> data);
    }
}
